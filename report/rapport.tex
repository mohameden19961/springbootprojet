...
{\LARGE \textbf{République Islamique de Mauritanie}}\\[0.3cm]
{\large Institut Supérieur du Numérique (SUPNUM) — Nouakchott}\\[1cm]
...
\begin{itemize}[leftmargin=*]
    \item \textbf{24068} - Abdy Ahmed Lebeid
    \item \textbf{24139} - Abd Salam Abdel Vetah
    \item \textbf{24238} - El Hassen Oumeiry
    \item \textbf{24014} - Haja Sid M'Hamed
    \item \textbf{24157} - Ahmedou Beddou
\end{itemize}
\end{flushleft}
\end{minipage}\\[1.5cm]
...
Nous remercions chaleureusement les membres de notre équipe de développement pour leur collaboration et leur engagement tout au long du projet :
...
Leur travail rigoureux et leur esprit d'équipe ont été essentiels à la réussite de ce projet.
...
\centering
\begin{tikzpicture}[
  entity/.style={draw, rectangle, minimum width=2.2cm, minimum height=0.7cm,
                 font=\small\bfseries, fill=blue!10},
  rel/.style={draw, diamond, aspect=2, minimum width=1.8cm,
              font=\scriptsize\bfseries, fill=yellow!20},
  attr/.style={draw, ellipse, minimum width=1.4cm, minimum height=0.5cm,
               font=\scriptsize},
  >=latex
]

% Ligne 1 : entités de référence
\node[entity] (LANG)  at (0,0)    {LANGUAGE};
\node[entity] (NAT)   at (4,0)    {NATIONALITY};
\node[entity] (CAT)   at (8,0)    {CATEGORY};
\node[entity] (PUB)   at (12,0)   {PUBLISHER};

% Ligne 2 : AUTHOR
\node[entity] (AUTH)  at (4,-3)   {AUTHOR};

% Ligne 3 : BOOK
\node[entity] (BOOK)  at (4,-6)   {BOOK};

% Ligne 4 : BOOK_ITEM et MEMBER
\node[entity] (BI)    at (1,-9)   {BOOK\_ITEM};
\node[entity] (MEM)   at (7,-9)   {MEMBER};

% Ligne 5 : BORROW et RESERVATION
\node[entity] (BOR)   at (1,-12)  {BORROW};
\node[entity] (RES)   at (7,-12)  {RESERVATION};

% Relations
\node[rel] (r_lang) at (2,-3)  {langue};
\node[rel] (r_nat)  at (4,-1.5){nationalité};
\node[rel] (r_cat)  at (6,-3)  {catégorie};
\node[rel] (r_pub)  at (8,-3)  {éditeur};
\node[rel] (r_ba)   at (1,-4.5){rédigé par};
\node[rel] (r_cont) at (1,-7.5){contient};
\node[rel] (r_bor)  at (4,-10.5){emprunte};
\node[rel] (r_res)  at (5.5,-7.5){réservé};

% Liens
\draw (LANG) -- (r_lang) -- (BOOK);
\draw (NAT)  -- (r_nat)  -- (AUTH);
\draw (CAT)  -- (r_cat)  -- (BOOK);
\draw (PUB)  -- (r_pub)  -- (BOOK);
\draw (BOOK) -- (r_ba)   -- (AUTH);
...
% Cardinalités
\node[font=\scriptsize] at (0.5,-2.2) {1,1};
\node[font=\scriptsize] at (2.5,-4)   {N,N};
\node[font=\scriptsize] at (2,-9.5)   {1,N};
\node[font=\scriptsize] at (6,-9.5)   {1,1};
...
\centering
\begin{tikzpicture}[
  actor/.style={draw, ellipse, minimum width=2.2cm, minimum height=0.8cm,
                font=\small\bfseries, fill=green!10},
  uc/.style={draw, ellipse, minimum width=4cm, minimum height=0.7cm,
...
\node[actor] (user)  at (12,0)   {Utilisateur};

% Système : rectangle
\draw[draw=black, thick] (2,-0.5) rectangle (10,-7);
\node[font=\small\itshape] at (6,0.2) {Système};

% Cas d'utilisation Admin (gauche)
\node[uc] (uc1) at (6,-1.2) {Gérer les livres};
\node[uc] (uc2) at (6,-2.2) {Gérer les auteurs};
\node[uc] (uc3) at (6,-3.2) {Gérer les membres};
\node[uc] (uc4) at (6,-4.2) {Gérer les catégories};
\node[uc] (uc5) at (6,-5.2) {Gérer les éditeurs};

% Cas d'utilisation User (droite)
\node[uc] (uc6)  at (6,-6.2) {Consulter le catalogue};

\node[uc] (uc7)  at (11,-2)  {Emprunter un livre};
\node[uc] (uc8)  at (11,-3)  {Retourner un livre};
\node[uc] (uc9)  at (11,-4)  {Renouveler un prêt};
\node[uc] (uc10) at (11,-5)  {Réserver un livre};
\node[uc] (vq)   at (11,-6)  {Vérifier quota};
...
% Include
\draw[dashed, ->] (uc7) -- node[above, font=\scriptsize]{<<include>>} (vq);
...
% Participants (en-têtes)
\node[draw, rectangle, minimum width=2.5cm, minimum height=0.7cm,
      fill=blue!15] (C)  at (0,0)    {Client HTTP};
\node[draw, rectangle, minimum width=2.5cm, minimum height=0.7cm,
      fill=blue!15] (CT) at (4,0)    {BorrowController};
\node[draw, rectangle, minimum width=2.5cm, minimum height=0.7cm,
      fill=blue!15] (S)  at (8,0)    {BorrowService};
\node[draw, rectangle, minimum width=2.5cm, minimum height=0.7cm,
      fill=blue!15] (R)  at (12,0)   {Repositories};
...
% Messages
\draw[->, thick] (0,-1)  -- node[above, font=\scriptsize]{POST /checkout} (4,-1);
\draw[->, thick] (4,-2)  -- node[above, font=\scriptsize]{borrowBook()} (8,-2);
\draw[->, thick] (8,-3)  -- node[above, font=\scriptsize]{findById(memberId)} (12,-3);
\draw[<-, thick] (8,-3.6)-- node[above, font=\scriptsize]{Member} (12,-3.6);
\draw[->, thick] (8,-4.5)-- node[above, font=\scriptsize]{findByBarcode()} (12,-4.5);
\draw[<-, thick] (8,-5.1)-- node[above, font=\scriptsize]{BookItem} (12,-5.1);
\draw[->, thick] (8,-6)  -- node[above, font=\scriptsize]{countByMemberAndStatus()} (12,-6);
\draw[<-, thick] (8,-6.6)-- node[above, font=\scriptsize]{activeBorrows} (12,-6.6);
\draw[->, thick] (8,-7.5)-- node[above, font=\scriptsize]{save(BORROWED)} (12,-7.5);
\draw[->, thick] (8,-8.2)-- node[above, font=\scriptsize]{save(Borrow)} (12,-8.2);
\draw[<-, thick] (8,-8.8)-- node[above, font=\scriptsize]{Borrow} (12,-8.8);
\draw[<-, thick] (4,-9.5)-- node[above, font=\scriptsize]{BorrowDTO} (8,-9.5);
\draw[<-, thick] (0,-10) -- node[above, font=\scriptsize]{201 Created} (4,-10);
...
\begin{enumerate}
    ...
    \item Projet Library Management System — Dépôt GitHub. \url{https://github.com/mohameden19961/springbootprojet}
\end{enumerate}
